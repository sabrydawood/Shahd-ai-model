...
const CodeExtension =
  /\.(html|css|scss|less|ts|tsx|js|jsx|go|py|rs|java|c|cpp|h|rb|php|cs|kt|swift|scala)$/;
    ...
    Fetch: async (Query: string, Limit: number): Promise<SourceInput[]> => {
      ...
      for (const Repo of Search.items ?? []) {
        ...
        const Files = (Tree.tree ?? [])
          .filter((E) => E.type === "blob" && CodeExtension.test(E.path))
          .slice(0, FilesPerRepo);
        for (const File of Files) {
          const Content = (await Http(
            `https://api.github.com/repos/${Repo.full_name}/contents/${encodeURIComponent(File.path)}`,
          )) as ContentResult;
          const Text =
            Content.encoding === "base64" && Content.content !== undefined
